...
public astInfo analyzeAST(set[Declaration] declarations) {
	...
	visit(declarations){
		 ...
		 * We have decided to consider single method (function) as unit.
			...
			individually."
		 */
	 	case Declaration x:constructor(_,parameters,_,implementation) : {
	 		units += getUnitInfo(x, parameters, implementation);
		}
		case Declaration x:method(_,_,parameters,_,implementation) : {
	    	units += getUnitInfo(x, parameters, implementation);
    	}
...
public unitInfo getUnitInfo(Declaration method, list[Declaration] parameters, Statement implementation){

		// lines count per unit (unit size)
		list[str] lines = readFileLines(method.src);
		...
		return <method.decl, lc, cc, size(parameters)>;
}
